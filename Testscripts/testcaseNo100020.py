...
tf = 'test_TestcaseNo100020'


# TestCaseNo:100020


class TestcaseNo100020(unittest.TestCase):
    def test_TestcaseNo100020(self):
        try:
            ...
            # Validate NetAmount
            abhibus_netmount = browser.find_element(By.ID, 'NetAmountmsg')
            abhibus_netamounttext = abhibus_netmount.text

            self.assertEqual(abhibus_netamounttext, '2,335.05')

        except Exception:
            traceback.print_exc()
            browser.save_screenshot(folder_path + '\Screenshots\Testcae-%s.png' % tf)
            self.fail('Test case No : 100020 is failed')
        finally:
